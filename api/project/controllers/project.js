    ...
    try {
        ...
        for (const project of projects) {
          for (const projectInterest of project.interests) {
            for (const projectCategories of projectInterest.categories) {
              if (
                projectCategories.category === userCategory.category &&
                !recommendedProjectSlugs.includes(project.slug)
              ) {
      ...
      */
      for (const recommendedProjectSlug of recommendedProjectSlugs) {
        const recommendedProject = await strapi.services.project.findOne({
          slug: recommendedProjectSlug,
        });
